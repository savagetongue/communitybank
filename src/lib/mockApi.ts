...
export const createRequest = (data: CreateRequestData): Promise<{ status: 'ok', requestId: string }> => {
  console.log('Simulating createRequest with data:', data);
  // In a real app, you would create a ServiceRequest record here.
  const requestId = `req-${Date.now()}`;
  return simulateDelay({ status: 'ok', requestId });
};
...
export const createBooking = (data: CreateBookingData): Promise<{ status: 'ok', bookingId: string }> => {
    console.log('Simulating createBooking with data:', data);
    const offer = mockOffers.find(o => o.id === data.offerId);
    const provider = mockProviders.find(p => p.id === offer?.providerId);
    if (!offer || !provider) {
        return Promise.reject(new Error('Offer or provider not found'));
    }
    const newBooking: Booking = {
        id: `booking-${Date.now()}`,
        requestId: `req-${Date.now()}`, // Correlate with a new request
        providerId: offer.providerId,
        memberId: 'user-demo', // Hardcoded for demo
        ...
        otherPartyAvatarUrl: provider.avatarUrl,
    };
    mockBookings.unshift(newBooking);
    return simulateDelay({ status: 'ok', bookingId: newBooking.id });
};