...
export async function setupDatabase(env: Env) {
    ...
    `);
    const [rows]: any = await db.execute('SELECT version FROM schema_version');
    ...
    if (currentVersion < SCHEMA_VERSION) {
        ...
        `);
        await db.execute('INSERT INTO schema_version (version) VALUES (?) ON DUPLICATE KEY UPDATE version = ?', [SCHEMA_VERSION, SCHEMA_VERSION]);
    }
    dbInitialized = true;
}
// --- Data Access Functions ---
export const findMemberByEmail = async (env: Env, email: string): Promise<Member | null> => {
    const [rows]: any = await getDb(env).execute('SELECT * FROM members WHERE email = ?', [email]);
    return rows[0] || null;
};
export const findMemberById = async (env: Env, id: string): Promise<Member | null> => {
    const [rows]: any = await getDb(env).execute('SELECT * FROM members WHERE id = ?', [id]);
    return rows[0] || null;
};
...
export const updateMember = async (env: Env, id: string, data: Partial<Member>): Promise<void> => {
    ...
    await getDb(env).execute(`UPDATE members SET ${fields} WHERE id = ?`, [...values, id]);
};
export const listOffers = async (env: Env, limit?: number): Promise<Offer[]> => {
    const query = `
        ...
        ${limit ? `LIMIT ${limit}` : ''}
    `;
    const [rows]: any = await getDb(env).execute(query);
...
export const findOfferById = async (env: Env, id: string): Promise<Offer | null> => {
    const [rows]: any = await getDb(env).execute(`
        SELECT o.*, m.name as providerName, m.avatarUrl as providerAvatarUrl, m.rating as providerRating
        FROM offers o
        JOIN members m ON o.providerId = m.id
        WHERE o.id = ?
    ...
    return { ...rows[0], skills: JSON.parse(rows[0].skills || '[]') };
};
...
export const listOffersByProviderId = async (env: Env, providerId: string): Promise<Offer[]> => {
    const [rows]: any = await getDb(env).execute('SELECT * FROM offers WHERE providerId = ? ORDER BY createdAt DESC', [providerId]);
    return rows.map(row => ({ ...row, skills: JSON.parse(row.skills || '[]') }));
};
...
export const findServiceRequestById = async (env: Env, id: string): Promise<ServiceRequest | null> => {
    const [rows]: any = await getDb(env).execute('SELECT * FROM service_requests WHERE id = ?', [id]);
    return rows[0] || null;
};
...
export const findBookingById = async (env: Env, id: string): Promise<Booking | null> => {
    const [rows]: any = await getDb(env).execute('SELECT * FROM bookings WHERE id = ?', [id]);
    return rows[0] || null;
};
export const updateBooking = async (env: Env, id: string, data: Partial<Booking>): Promise<void> => {
    const fields = Object.keys(data).map(key => `${key} = ?`).join(', ');
    const values = Object.values(data);
    await getDb(env).execute(`UPDATE bookings SET ${fields} WHERE id = ?`, [...values, id]);
};
export const listBookingsByUserId = async (env: Env, userId: string): Promise<any[]> => {
    ...
    `;
    const [rows]: any = await getDb(env).execute(query, [userId, userId, userId]);
    return rows;
};
export const getLastLedgerEntry = async (env: Env, memberId: string): Promise<LedgerEntry | null> => {
    const [rows]: any = await getDb(env).execute('SELECT * FROM ledger_entries WHERE memberId = ? ORDER BY createdAt DESC LIMIT 1', [memberId]);
    return rows[0] || null;
};
...
export const listLedgerByUserId = async (env: Env, userId: string): Promise<LedgerEntry[]> => {
    const [rows]: any = await getDb(env).execute('SELECT * FROM ledger_entries WHERE memberId = ? ORDER BY createdAt DESC', [userId]);
    return rows;
};
...
export const findDisputeById = async (env: Env, id: string): Promise<Dispute | null> => {
    const [rows]: any = await getDb(env).execute('SELECT * FROM disputes WHERE id = ?', [id]);
    return rows[0] || null;
};
export const updateDispute = async (env: Env, id: string, data: Partial<Dispute>): Promise<void> => {
    const fields = Object.keys(data).map(key => `${key} = ?`).join(', ');
    const values = Object.values(data);
    await getDb(env).execute(`UPDATE disputes SET ${fields} WHERE id = ?`, [...values, id]);
};